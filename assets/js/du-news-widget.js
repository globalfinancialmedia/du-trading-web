...
var duNewsBaseUrl = 'https://api.du-uat.mubashertrade.com:8080';

var info_news_widget_urls = [
    { "slug": "du_top_news", "url": duNewsBaseUrl + "/newsservice/" + duLangState + "/TOP_NEWS/" },
    { "slug": "du_most_read", "url": duNewsBaseUrl + "/newsservice/" + duLangState + "/MOST_READ/" },
    { "slug": "du_press_release", "url": duNewsBaseUrl + "/newsservice/" + duLangState + "/PRESS_RELEASE/" },
    { "slug": "du_international_market", "url": duNewsBaseUrl + "/newsservice/" + duLangState + "/INTERNATIONAL_MARKET/" },
    { "slug": "du_arab_market", "url": duNewsBaseUrl + "/newsservice/" + duLangState + "/ARAB_MARKET/" },
    { "slug": "du_islamic_finance", "url": duNewsBaseUrl + "/newsservice/" + duLangState + "/ISLAMIC_FINANCE/" },
    { "slug": "du_news_exclusive", "url": duNewsBaseUrl + "/newsservice/" + duLangState + "/MUBASHER_EXCLUSIVE/" },
];
...
function loadInfoJsWidget(widget_data) {
    widget_data.widgets.forEach((config_data) => {
        ...
        info_req_headers = Object.assign({ "authorization": 'Bearer ' + widget_data.authToken }, info_req_headers);
...
function calculateNewsTime(date) {
    var DuNewsTime = "";
    var DuNewsCalculatedTime = Math.abs(new Date().getTime() - new Date(date).getTime());
    var duNewsMinutes = Math.floor(DuNewsCalculatedTime / 60000);
    var duNewsHours = Math.floor(duNewsMinutes / 60);
    var duNewsDays = Math.floor(duNewsHours / 24);
    var duNewsMonth = Math.floor(duNewsDays / 30);
    switch (duStateLanguage) {
        default:
            if (duNewsMonth >= 1) {
                if (duNewsMonth === 1) {
                    return DuNewsTime = duNewsMonth + ' month ago';
                } else {
                    return DuNewsTime = duNewsMonth + ' months ago';
                }
            } else if (duNewsDays >= 1) {
                if (duNewsDays === 1) {
                    return DuNewsTime = duNewsDays + ' day ago';
                } else {
                    return DuNewsTime = duNewsDays + ' days ago';
                }
            } else if (duNewsHours >= 1) {
                if (duNewsHours === 1) {
                    return DuNewsTime = duNewsHours + ' hour ago';
                } else {
                    return DuNewsTime = duNewsHours + ' hours ago';
                }
            } else if (duNewsMinutes >= 1) {
                if (duNewsMinutes === 1) {
                    return DuNewsTime = duNewsMinutes + ' minute ago';
                } else {
                    return DuNewsTime = duNewsMinutes + ' minutes ago';
                }
            }
            break;
        case 'AR':
            if (duNewsMonth >= 1) {
                if (duNewsMonth === 1) {
                    return DuNewsTime = duNewsMonth + ' منذ شهر';
                } else if (duNewsMonth === 2) {
                    return DuNewsTime = duNewsMonth + ' منذ شهران';
                } else {
                    return DuNewsTime = duNewsMonth + ' منذ اشهر';
                }
            } else if (duNewsDays >= 1) {
                if (duNewsDays === 1) {
                    return DuNewsTime = duNewsDays + ' منذ يوم';
                } else if (duNewsDays === 2) {
                    return DuNewsTime = duNewsDays + ' منذ يومان';
                } else {
                    return DuNewsTime = duNewsDays + ' منذ ايام';
                }
            } else if (duNewsHours >= 1) {
                if (duNewsHours === 1) {
                    return DuNewsTime = duNewsHours + ' منذ ساعة';
                } else if (duNewsHours === 2) {
                    return DuNewsTime = duNewsHours + ' منذ ساعتان';
                } else {
                    return DuNewsTime = duNewsHours + ' منذ ساعات';
                }
            } else if (duNewsMinutes >= 1) {
                if (duNewsMinutes === 1) {
                    return DuNewsTime = duNewsMinutes + ' منذ دقيقة';
                } else if (duNewsMinutes === 2) {
                    return DuNewsTime = duNewsMinutes + ' منذ دقيقتان';
                } else {
                    return DuNewsTime = duNewsMinutes + ' منذ دقائق';
                }
            }
            break;
    }
    ...
    switch (duStateLanguage) {
        default:
            ...
            responseDataObj.forEach(data => {
                ...
                <small class="title-under-img">Markets - <span>` + calculateNewsTime(data.updatedAt) + `</span></small><h5 class="title-img-description">` + data.title + `</h5></a><hr></div>`
            });
            outputHtmlString = `<a href="#" onClick="viewDuNewsPage('summaryPage',` + responseDataObj[0].id + `, event);" class="lnd-tp-news lnd-tp-news-des">
                ...
                <span class="lnd-tp-news-time font14 du-text-white">` + responseDataObj[0].source.name.stringEn + ` - ` + calculateNewsTime(responseDataObj[0].updatedAt) + `</span></a><div class="du-row du-mt-3">
                ...
                <span class="lnd-tp-news-time font14 du-text-white">Markets - ` + calculateNewsTime(responseDataObj[0].updatedAt) + `</span>
        ...
        case 'AR':
            ...
            responseDataObj.forEach(data => {
                ...
                <small class="title-under-img">محتوی الشریک - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
                <h5 class="title-img-description">` + data.title + `</h5></a><hr></div>`
            });
            outputHtmlStringAr = `<a href="#" onClick="viewDuNewsPage('summaryPage',` + responseDataObj[0].id + `, event);" class="lnd-tp-news"><img class="du-img-fluid" src=` + srcImgSet + `>
            ...
            <span class="lnd-tp-news-time font14 du-text-white">` + calculateNewsTime(responseDataObj[0].updatedAt) + `</span></a><div class="du-row du-mt-3">` + insideLoopHTMLAr + `</div>`;
...
function mostReadNewsHTML(responseDataObj) {
    switch (duStateLanguage) {
        ...
        case 'AR':
            ...
            outputHtmlStringAr = `<div class="most-read-news-list"><h4 class="sidebar-readmore-title du-black-title">الأكثر قراءة</h4>` + insideLoopHTMLAr + `<div class="du-row">
...
function exclusiveNewsHomeHTML(responseDataObj) {
    switch (duStateLanguage) {
        default:
            ...
            let insideLoopHTML = "";
            responseDataObj.forEach(data => {
                insideLoopHTML += `<div class="du-col-md-4">
                <img class="du-img-fluid img-width-100" src=` + data.image.primary + `><small class="title-under-img du-black-title">` + data.source.name.stringEn + ` - <span>` + calculateNewsTime(data.updatedAt) + `</span>
                </small><a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);"><h5 class="title-img-description">` + data.title + `</h5></a><hr></div>`
            });
            outputHtmlString = `<h4 class="exclusive-heading mt70">Mubasher Exclusive</h4><a href="#" class="view-all-text" onClick="duListingNewsPage('summaryPage', event, 'MUBASHER_EXCLUSIVE');">See all
        ...
        case 'AR':
            ...
            let insideLoopHTMLAr = "";
            responseDataObj.forEach(data => {
                insideLoopHTMLAr += `<div class="du-col-md-4">
                <img class="du-img-fluid img-width-100" src=` + data.image.primary + `>
                <small class="title-under-img">محتوی الشریک - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
                <a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);"><h5 class="title-img-description">` + data.title + `</h5></a><hr></div>`
            });
            outputHtmlStringAr = `<h4 class="exclusive-heading mt70">حصرياً مباشر</h4><a href="#" class="view-all-text" onClick="duListingNewsPage('summaryPage', event, 'MUBASHER_EXCLUSIVE');">اظهار الكل <img class="pr-1 rotate180" 
    ...
    switch (duStateLanguage) {
        default:
            ...
            responseDataObj.forEach(data => {
                ...
                <img class="du-img-fluid du-w-100" src=` + data.image.primary + `>
                </div><div class="du-col-sm-8"><small>` + data.source.name.stringEn + ` - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
                <a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);"><h6>` + data.title + `</h6></a></div></div></div>`
            });
            outputHtmlString = `<h4 class="exclusive-heading">Islamic Finance</h4><a href="#" class="view-all-text" onClick="duListingNewsPage('summaryPage', event, 'ISLAMIC_FINANCE');">See all
        ...
        case 'AR':
            ...
            responseDataObj.forEach(data => {
                ...
                <img class="du-img-fluid w-100" src=` + data.image.primary + `></div><div class="du-col-sm-8">
                <small class="title-under-img">محتوی الشریک - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
                <a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);"><h6 class="title-img-description">` + data.title + `</h6></a></div></div></div>`
            });
            outputHtmlStringAr = `<h4 class="exclusive-heading">اقتصاد إسلامي</h4><a href="#" class="view-all-text" onClick="duListingNewsPage('summaryPage', event, 'ISLAMIC_FINANCE');">اظهار الكل<img class="pr-1 rotate180" 
...
function arabMarketNewsHTML(responseDataObj) {
    switch (duStateLanguage) {
        default:
            ...
            let insideLoopHTML = "";
            responseDataObj.forEach(data => {
                console.log('data arab markets', data);
                insideLoopHTML += `<div class="isl-fi-news-wrap du-col-sm-4">
                <img class="du-img-fluid width-100 img-width-100" src=` + data.image.primary + `>
                <div class="clearfix du-mt-3"><small class="title-under-img du-black-title">` + data.source.name.stringEn + ` - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
                <a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);"><h6 class="title-img-description">` + data.title + `</h6></a></div><hr></div>`
            });
            outputHtmlString = `<h4 class="exclusive-heading">Arab Market</h4><a href="#" class="view-all-text" onClick="duListingNewsPage('summaryPage', event, 'ARAB_MARKET');">See all
        ...
        case 'AR':
            ...
            let insideLoopHTMLAr = "";
            responseDataObj.forEach(data => {
                insideLoopHTMLAr += `<div class="isl-fi-news-wrap du-col-sm-4">
                <img class="du-img-fluid width-100 img-width-100" src=` + data.image.primary + `><div class="clearfix du-mt-3">
                <small class="title-under-img du-black-title">محتوی الشریک - <span>` + calculateNewsTime(data.updatedAt) + `</span>
                </small><a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);"><h6 class="title-img-description">` + data.title + `</h6></a>
                </div><hr></div>`
            });
            outputHtmlStringAr = `<h4 class="exclusive-heading">الأسواق العربية</h4><a href="#" class="view-all-text" onClick="duListingNewsPage('summaryPage', event, 'ARAB_MARKET');">اظهار الكل<img class="pr-1 rotate180" 
...
function internationalMarketHTML(responseDataObj) {
    switch (duStateLanguage) {
        default:
            ...
            let insideLoopHTML = "";
            responseDataObj.forEach(data => {
                insideLoopHTML += `<div class="du-row"><div class="du-col-md-4"><img class="du-img-fluid width-100" src=` + data.image.primary + `></div>
                <div class="du-col-md-8 du-mt-4"><small class="title-under-img du-black-title">` + data.source.name.stringEn + ` - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
            ...
            outputHtmlString = `<h4 class="exclusive-heading">International Market</h4><a href="#" class="view-all-text" onClick="duListingNewsPage('summaryPage', event, 'INTERNATIONAL_MARKET');">See all<img class="pl-1" src="https://du-assets-bucket.s3-eu-west-1.amazonaws.com/du/assets/images/path-487.svg" alt=""></a>` + insideLoopHTML;
            return outputHtmlString;
            break;
        case 'AR':
            let outputHtmlStringAr = "";
            let insideLoopHTMLAr = "";
            responseDataObj.forEach(data => {
                insideLoopHTMLAr += `<div class="du-row"><div class="du-col-md-4">
                <img class="du-img-fluid width-100" src=` + data.image.primary + `>
                </div><div class="du-col-md-8"><small class="title-under-img du-black-title">محتوی الشریک - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
                <a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);"><h5 class="title-img-description">` + data.title + `</h5></a>
                <p>` + data.teaser + `</p></div></div><hr>`
            });
            outputHtmlStringAr = `<h4 class="exclusive-heading">الأسواق العالمية</h4><a href="#" onClick="duListingNewsPage('summaryPage', event, 'INTERNATIONAL_MARKET');" 
...
function pressReleaseNewsHTML(responseDataObj) {
    switch (duStateLanguage) {
        default:
            ...
            var srcImgSet = responseDataObj[0].image.srcset.split(',')[2];
            responseDataObj.forEach(data => {
                insideLoopHTML += `<div class="press-rel-news"><small class="title-under-img du-black-title">` + data.source.name.stringEn + ` - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
        ...
        case 'AR':
            ...
            var srcImgSet = responseDataObj[0].image.srcset.split(',')[2];
            responseDataObj.forEach(data => {
                insideLoopHTMLAr += `<a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);"><div class="press-rel-news du-mt-3"><small class="title-under-img">محتوی الشریک - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
                <h5 class="title-img-description">` + data.title + `</h5></div></a><hr>`
            });
            outputHtmlStringAr = `<h4 class="exclusive-heading">بيانات صحفية</h4><a href="#" class="view-all-text" onClick="duListingNewsPage('summaryPage', event, 'PRESS_RELEASE');">اظهار الكل<img class="pr-1 rotate180" 
    ...
    switch (duStateLanguage) {
        default:
            ...
            responseDataObj.forEach(data => {
                ...
                <img class="du-img-fluid width-100" src=` + data.image.primary + ` alt=""></div><div class="du-col-sm-8 du-mt-4"><small class="title-under-img du-black-title">` + data.source.name.stringEn + ` - 
                <span>` + calculateNewsTime(data.updatedAt) + `</span></small><a href="#" onClick="viewDuNewsPage('listingPage',` + data.id + `, event);"><h4 class="title-news-description">`
            ...
            <div class="news-inner"><div class="bg-clr-fr-news d-flex flex-wrap align-content-center">
            <img class="du-img-fluid img-width-100" src=` + responseDataObj[0].image.primary + ` alt=""></div><div class="clearfix du-mt-3">
            <small class="title-under-img du-black-title">Markets - <span>` + calculateNewsTime(responseDataObj[0].updatedAt) + `</span></small><h3 class="title-img-description du-black-title">
            ...
            <div class="du-col-sm-6"><div class="news-inner"><img class="du-img-fluid img-width-100" src=` + responseDataObj[1].image.primary + ` alt="">
            <div class="clearfix du-mt-3"><small class="title-under-img du-black-title">Markets - <span>` + calculateNewsTime(responseDataObj[1].updatedAt) + `</span></small><h3 class="title-img-description du-black-title">
            <a class="title-img-description txt-black" href="#" onClick="viewDuNewsPage('listingPage',` + responseDataObj[1].id + `, event);">` + responseDataObj[1].title + `</a></h3></div></div></div></div>` + insideLoopHTML +
                `<div class="du-row"><div class="du-col-sm-12 mt-4 mb-5 du-text-center"><a href="#" class="main-color ff-hel-b" onclick="paginateNews('du_news_view_listing', event)">View More<span>
        ...
        case 'AR':
            ...
            responseDataObj.forEach(data => {
                ...
                <div class="du-col-sm-8"><small class="title-under-img">` + data.source.name.stringEn + ` - <span>` + calculateNewsTime(data.updatedAt) + `</span></small>
                <a href="#" onClick="viewDuNewsPage('listingPage',` + data.id + `, event);">
                <h4 class="title-news-description">`+ data.title + `</h4></a><p>` + data.teaser + `</p></div><div class="du-col-12"><hr></div></div>`
            });
            outputHtmlStringAr = `<h1 class="main-news-title mt-4 du-black-title">` + responseDataObj[0].title + `</h1><div class="du-row"><div class="du-col-sm-6">
            <div class="news-inner"><img class="du-img-fluid img-width-100" src=` + responseDataObj[0].image.primary + `  alt=""><div class="clearfix mt-3">
            <small class="title-under-img du-black-title">محتوی الشریک - <span>` + calculateNewsTime(responseDataObj[0].updatedAt) + `</span></small>
            <a href="#" onClick="viewDuNewsPage('listingPage',` + responseDataObj[0].id + `, event);">
            <h3 class="title-img-description du-black-title">` + responseDataObj[0].title + `</h3></a></div></div></div>
            <div class="du-col-sm-6"><div class="news-inner"><img class="du-img-fluid img-width-100" src=` + responseDataObj[1].image.primary + ` alt="">
            <div class="clearfix mt-3"><small class="title-under-img">محتوی الشریک - <span>` + calculateNewsTime(responseDataObj[1].updatedAt) + `</span></small>
            <a href="#" onClick="viewDuNewsPage('listingPage',` + responseDataObj[1].id + `, event);">
            <h3 class="title-img-description du-black-title">`+ responseDataObj[1].title + `</h3></a></div></div>
            </div></div>` + insideLoopHTMLAr + `<div class="du-row"><div class="du-col-sm-12 mt-4 mb-5 du-text-center"><a href="#" class="main-color ff-hel-b" onclick="paginateNews('du_news_view_listing', event)">تحميل المزيد <span>
            <img src="https://du-assets-bucket.s3-eu-west-1.amazonaws.com/du/assets/images/path-392.svg" alt=""></span></a></div></div>`;
            return outputHtmlStringAr;
            break;
    }
...
function viewNewsDetailHTML(responseDataObj) {
    switch (duStateLanguage) {
        default:
            ...
            <small class="pull-right">` + responseDataObj.source.name.stringEn + ` - <span>` + calculateNewsTime(responseDataObj.updatedAt) + `</span></small></div></div><div class="du-row">
        ...
        case 'AR':
            ...
            <small class="news-under-title-description">محتوی الشریک - <span>` + calculateNewsTime(responseDataObj.updatedAt) + `</span></small><img class="img-fluid du-mb-3 du-mt-2 width-100" 
    ...
    switch (duStateLanguage) {
        default:
            ...
            responseDataObj.forEach(data => {
                ...
                <img class="du-img-fluid width-100" src=` + data.image.primary + ` alt="">
                </div><div class="du-col-sm-8"><small class="du-black-title">Markets - <span>` + calculateNewsTime(data.updatedAt) + `</span></small><br><br><a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);">
        ...
        case 'AR':
            ...
            responseDataObj.forEach(data => {
                ...
                محتوی الشریک - <span>` + calculateNewsTime(data.updatedAt) + `</span></small><a href="#" onClick="viewDuNewsPage('summaryPage',` + data.id + `, event);">
                <h4 class="title-news-description">` + data.title + `</h4></a><p>` + data.teaser + `</p></div>
...
function viewDuNewsPage(state, id, event) {
    ...
    info_news_widget_urls = [
        { "slug": "du_related_news_view", "url": duNewsBaseUrl + "/newsservice/" },
        { "slug": "du_related_news", "url": duNewsBaseUrl + "/newsservice/" + duLangState + "/" }
    ];
    loadInfoJsWidget(info_widgets_config_data);
};

function duListingNewsPage(state, event, category) {
    event.preventDefault();
    sessionStorage.setItem('du-widget-state', state);
    var innerContent = document.getElementById("du_news_page");
    innerContent.innerHTML = "";
    ...
    info_news_widget_urls = [];
    info_news_widget_urls = [
        { "slug": "du_news_view_listing", "url": duNewsBaseUrl + "/newsservice/" + duLangState + "/" + category + "/" }
    ];
    loadInfoJsWidget(info_widgets_config_data);
};
